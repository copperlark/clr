...
// for sha2
//uint256 hashGenesisBlock("0x000000009f5a760455871360f4d40c6a0dad0f2af30233e6d06536ebef5b485b");
// for sha3 keccak
...
const string strMessageMagic = "CopperLark Signed Message:\n";
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CopperLarkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CopperLarkMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("CopperLarkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);    

    RenameThread("CopperLark-miner");
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running CopperLarkMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d CopperLarkMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBitcoinMiner, pwallet))
                printf("Error: NewThread(ThreadCopperLarkMiner) failed\n");
            Sleep(10);
        }
    }
}
